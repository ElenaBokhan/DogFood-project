export enum ETestId {
    ADD_PRODUCT_NAME_INPUT = 'addProductNameInput',
    ADD_PRODUCT_PRICE_INPUT = 'addProductPriceInput',
    ADD_PRODUCT_DESCRIPTION_INPUT = 'addProductDescriptionInput',
    ADD_PRODUCT_DISCOUNT_INPUT = 'addProductDiscountInput',
    ADD_PRODUCT_STOCK_INPUT = 'addProductStockInput',
    ADD_PRODUCT_WIGHT_INPUT = 'addProductWightInput',
    ADD_PRODUCT_PICTURES_INPUT = 'addProductPicturesInput',
    ADD_PRODUCT_SUBMIT_BUTTON = 'addProductSubmitButton',

    ADD_REVIEW_SUBMIT_BUTTON = 'addReviewSubmitButton',
    ADD_REVIEW_TEXT_INPUT = 'addReviewTextInput',

    FOOTER_MAIN_LOGO = 'footerMainLogo',
    FOOTER_MENU = 'footerMenu',
    FOOTER_CONTACTS = 'footerContacts',

    HEADER_MAIN_LOGO = 'headerMainLogo',
    HEADER_ADD_NEW_ICON = 'headerAddNewProduct',
    HEADER_FAVOURITES_ICON = 'headerFavouritesIcon',
    HEADER_CART_ICON = 'headerCartIcon',
    HEADER_PROFILE_ICON = 'headerProfileIcon',
    HEADER_LOG_OUT_ICON = 'headerLogOutIcon',
    HEADER_SEARCH_INPUT = 'headerSearchInput',
}
